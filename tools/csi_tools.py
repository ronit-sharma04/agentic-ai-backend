...
def update_cases_tool(inputs: CSIToolArgs) -> dict:
    """
    Update CSI (Customer Shipment Instruction) records based on provided filters and updated field values.

    This tool identifies a CSI record using any combination of CSI fields (like `read_cases_tool`) and updates 
    the first matching document with the new values provided.

    === Capabilities ===
    - Flexible filter: uses any field to locate a matching CSI record.
    - Fields used for filtering will not be updated; only remaining fields will be changed.
    - Supports partial updates — you only need to provide what you want to change.
    - Skips empty or null fields automatically.
    - Returns a status message and the updated record.

    === Filterable & Updatable Fields ===
    You can provide any of the following fields:
    ...
    Example Usage:
    Use this tool to update any CSI field by providing identifying filters and new values.
    """

    try:
        data = {k: v for k, v in inputs.model_dump().items() if v not in ("", None)}
        print(f"[CSI UPDATE TOOL] Called with inputs={data}")
...
update_cases_tool.name = "csi_update_tool"
