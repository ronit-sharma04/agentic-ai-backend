...
def get_fields_without_heading():
    data = fetch_business_ruleset()
    lines = data.strip().splitlines()
    print(lines)

    # Remove the first line if it says "Mandatory Fields:"
    if lines and lines[0].strip().lower().startswith("mandatory fields:"):
        lines = lines[1:]
...
def get_form_fields():
    mandatory_fields = get_fields_without_heading()
    form_fields = {
    ...
    return {
            "message": "Form field definitions loaded successfully",
            "data": {
